...
function renderInfos(container, inscription, showView) {
  ...
  container.innerHTML = ''
    ...
    + '</div>'
    + '<div class="usager-card">'
    +   '<div class="usager-card-title">Venue sans réservation</div>'
    +   '<p class="usager-infos-text">La venue sur la plage est possible <strong>sans réservation préalable</strong>. Pour profiter d\'un maximum de temps, il est conseillé d\'arriver en <strong>début de créneau</strong>.</p>'
    +   '<p class="usager-infos-text">Si toutes les places sont occupées à votre arrivée, vous devrez <strong>patienter qu\'un emplacement se libère</strong>. Les usagers ayant réservé sont prioritaires.</p>'
    +   '<div class="usager-infos-rule">'
    +     '<img src="icone%20horaire.svg" class="usager-infos-rule-icon" alt="">'
    +     '<span>Toute réservation non honorée dans les <strong>30 minutes suivant le début du créneau</strong> est considérée comme caduque, la place est alors libérée pour les personnes présentes sur place.</span>'
    +   '</div>'
    + '</div>'
    + '<div class="usager-card">'
