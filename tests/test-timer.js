...
const { DURATION_MS, DURATION_MS_DOUBLE, getTimeRemaining, formatCountdown, getUrgencyLevel } = require('../js/timer.js');

// Constants
assert.strictEqual(DURATION_MS, 105 * 60 * 1000);
assert.strictEqual(DURATION_MS_DOUBLE, 210 * 60 * 1000);

// getTimeRemaining — arrivée à l'instant (sans durationMs = default 1h45)
{
  const now = Date.now();
...
// getTimeRemaining — arrivée à l'instant avec durationMs explicite
{
  const now = Date.now();
  const remaining = getTimeRemaining(now, DURATION_MS_DOUBLE);
  assert.ok(remaining >= DURATION_MS_DOUBLE - 10 && remaining <= DURATION_MS_DOUBLE, 'should be ~DURATION_MS_DOUBLE');
}

// getTimeRemaining — arrivée il y a 30 minutes (simple)
{
...
// getTimeRemaining — double créneau, arrivée il y a 30 minutes
{
  const checkin = Date.now() - 30 * 60 * 1000;
  const remaining = getTimeRemaining(checkin, DURATION_MS_DOUBLE);
  const expected = 180 * 60 * 1000;
  assert.ok(Math.abs(remaining - expected) < 100, 'should be ~180 minutes remaining');
}
...
assert.strictEqual(getUrgencyLevel(5 * 60 * 1000),          'critical');
assert.strictEqual(getUrgencyLevel(14 * 60 * 1000 + 59000), 'critical');
assert.strictEqual(getUrgencyLevel(15 * 60 * 1000),         'warning');
assert.strictEqual(getUrgencyLevel(29 * 60 * 1000),         'warning');
assert.strictEqual(getUrgencyLevel(30 * 60 * 1000),         'ok');
